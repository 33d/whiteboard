...
translate([0, 0, 15]) difference() {
  translate([0, 0, -15]) rotate([90, 0, 0]) union() {
    cube(size=[100, 15, 20]);
    translate([20, 0, 0]) rotate([0, -90, 0]) cube(size=[100, 15, 20]);
  }
  rotate([180, 0, 0]) translate([10, 10, 0.001]) pinhole(h=10, tight=false);
}

translate([40, -80, 0]) pinpeg(h=35, tight=false);

rotate([180, 0, 0]) translate([40, 40, -15]) difference() {
  rotate_extrude(file="pulley-side.dxf");
  pinhole(h=10, tight=false);
}
